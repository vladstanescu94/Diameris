...
| Domain/Utilities buildable on macOS | 2026-08-06 | Added `.macOS(.v26)` to `Domain` and `Utilities` `platforms:`; wrapped `HapticManager.swift` and `KeyboardHelper.swift` in `#if canImport(UIKit)`. Additive only — iOS build byte-for-byte unaffected. Enables the local web client to reuse the real `TransferCalculator` and `AmountFormatter` instead of reimplementing them. Note: SwiftPM *copies* `Localizable.xcstrings` without compiling it on macOS, so `Bundle.module` lookups fall back to the key — harmless, since the Domain keys *are* the English strings |
